...
if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "Usage: bash run.sh MEDIA_FILE [MODEL]" >&2
  echo 'Example: bash run.sh "recording.mp4" small.en' >&2
  exit 1
fi
if [[ ! -f "$1" ]]; then
...
if ! docker info >/dev/null 2>&1; then
  echo "Error: Docker is unavailable. Start Docker and check your Docker permissions." >&2
  exit 1
fi
...
docker build --tag transcribe:local "$SCRIPT_DIR/lib"
docker run --rm \
  --user "$(id -u):$(id -g)" \
